...
// @namespace .
// @include https://iop.portal.nttltd.global.ntt/*
// @version Last one
// @grant none
// ==/UserScript==
...
if( action == "my-work" ){
	...
	if (typeof team_order === 'undefined'){team_order=[
		'ISC GNS',
    	'ISC Data Center Services',
    	'ISC Cloud Comms',
    	'ISC 24x7',
        'ISC Shared Infrastructure Services',
    	'ISC 3rd parties',
    	'ISC Global Networks',
    	'ISC Network Services',
    	'ISC Pending RFO',
        'Abuse'
	]};
	team_order_delegate=[
	...
	if ( typeof team_colors === 'undefined' ) {
    	team_colors = {
    		'ISC GNS': "#F781BE",//"#66CCFF"
    		'ISC Data Center Services': "gray",
    		'ISC Cloud Comms': "yellow",
    		'ISC 24x7': "orange",
        	'ISC Shared Infrastructure Services': "lightblue",
    		//'GSD Integrated Services ES': "red",
    		'ISC 3rd parties': "#9494FF",
    		'ISC Global Networks': "#cd76ff",
    		'ISC Network Services': "#C2A584",
    		'ISC Pending RFO': "#E9C969",
        	'Abuse': "red"
    	};
    }
...
if ("view-tickets"==action||"view-tasks"==action){
	var i=0;$("td a:contains('Portal Response'),a:contains('Outbound Mail'), a:contains('Inbound Mail')").parent().next().each(function(){this.id||(this.id="class1cell"+i);
	var e=this.innerHTML;if(-1==e.indexOf("From: ")||e.indexOf("From: ")>e.indexOf("To: "))if(-1==e.indexOf("To: "))var t="",s="",n="",l="",c="",d="";
	else{
		var s="",t=e.split("To: ",2);
		if(-1==e.indexOf("Cc: ")||e.indexOf("Cc: ")>e.indexOf("Subject: "))var l="",n=t[1].split("Subject: ",2),c=n[1].split("<br><br>",5);
		else var n=t[1].split("Cc: ",2),l=n[1].split("Subject: ",2),c=l[1].split("<br><br>",5)}
		else{
			var t=e.split("From: ",2),s=t[1].split("To: ",2);if(-1==e.indexOf("Cc: ")||e.indexOf("Cc: ")>e.indexOf("Subject: "))var n=s[1].split("Subject: ",2),c=n[1].split("<br><br>",5);
			else var n=s[1].split("Cc: ",2),l=n[1].split("Subject: ",2),c=l[1].split("<br><br>",5)}
		if(""!=c){this.innerHTML="";
		var r=document.createElement("DIV"),p=document.createElement("DIV"),h=document.createElement("BUTTON"),o=document.createElement("BUTTON"),b=document.createElement("P"),f=document.createElement("P");
		if(r.id="div"+i,p.id="sumdiv"+i,b.id="p"+i,f.id="sump"+i,h.id="butshow"+i,o.id="buthide"+i,h.innerHTML="Show",o.innerHTML="Hide",h.addEventListener("click",function(){$(this).next().next().css("display","block").css("width","426px"),$(this).next().next().next().css("display","none"),$(this).css("display","none"),$(this).next().css("display","initial")}),o.addEventListener("click",function(){$(this).next().css("display","none"),$(this).next().next().css("display","block").css("width","426px"),$(this).css("display","none"),$(this).prev().css("display","initial")}),this.appendChild(h),this.appendChild(o),this.appendChild(r),this.lastChild.appendChild(b),b.innerHTML=e,this.appendChild(p),this.lastChild.appendChild(f),""!=s)if(0==s[0].indexOf('"')){s[0]=s[0].slice(1,-4);var x=s[0].indexOf(" ");
		d=s[0].slice(0,x-1)+" --> "}
		else if(0==s[0].indexOf("&lt;")){s[0]=s[0].slice(4,-3);var x=s[0].indexOf(" ");
		d=s[0].slice(0,x-4)+" --> "}else d=s[0]+" --> ";
		else d="NTT EU --> ";
		var u=n[0];for(u=u.replace(/<br\s*[\/]?>/gi," "),u=u.replace(/\t/gi,""),u=u.replace(/&lt;/gi,""),u=u.replace(/&gt;/gi,""),u=u.replace(/,/gi,""),u=u.replace(/;/gi,""),u=u.replace(/"/gi,""),u=u.replace(/\'/gi,""),u=u.toLowerCase(),u=u.split(" "),a=0;a<u.length;a++)u[a]!=u[a-1]&&u[a].indexOf("@")>-1&&(d+=u[a],a+2<u.length&&(d+=", ")),a==u.length-1&&u.length>1&&(d+="<br/>");d+="<br/>";var v="";c[1]?v+=c[1].replace(/<br>/gi,"")+" ":"",c[2]?v+=c[2].replace(/<br>/gi,"")+" ":"",c[3]?v+=c[3].replace(/<br>/gi,"")+" ":"",c[4]?v+=c[4].replace(/<br>/gi,"")+" ":"",v.length>350?(d+=v.slice(0,350),d+="..."):d+=v,f.innerHTML=d}$("#div"+i).css("display","none"),$("#sumdiv"+i).css("width","426px"),$("#buthide"+i).css("display","none"),i++})}
...
// Runbook Overlay
if(ovrunbook){
	$("div.task_action_content").css("position","relative"),
	$("head").append("<style>.b-popup-content{margin:20px auto 0;width:550px;height:310px;padding:10px;background-color:#f5f5f5;border-radius:5px;box-shadow:0 0 10px #000;}.popupTextarea{display:none;width:99%; height:180px;border: 1px solid black;border-radius:2px;}.runbookYes{margin: 3px 2px 3px 22%;display:inline-block;border:1px solid #BFBFBF;color:#8C8C8C;border-radius:20px;-webkit-border-radius:20px;-moz-border-radius:20px;font-family:Verdana;width:auto;height:auto;font-size:16px;padding:10px 40px;background-color:#FCFAF9}.runbookYes:active,.runbookYes:hover{border:1px solid #BFBFBF;color:#1CD100;background-color:#E8E8E8}.runbookNo{display:inline-block;border:1px solid #BFBFBF;color:#8C8C8C;border-radius:20px;-webkit-border-radius:20px;-moz-border-radius:20px;font-family:Verdana;width:auto;height:auto;font-size:16px;padding:10px 40px;background-color:#FCFAF9}.runbookNo:active,.runbookNo:hover{border:1px solid #BFBFBF;color:#ED0202;background-color:#E8E8E8}.runbookGo{display:none;width: 56%;height: 40px;margin: -5px 0px 9px 22%;font-size: 20px;}.popupQuestion{font-size:20px; color:black;margin:0 0 0 7%;text-transform: uppercase;text-align: center;}#transp {position:absolute;top:0;left:0;background: rgba(0,0,0,.5);width:100%;height:100%;z-index:10;}</style>");
	var runbook=$('td:contains("Runbook:")'),
	from=$("#work-log tr.row1:first").text();
	runbook.length&&-1!=frow.indexOf("Changed Category")&&($("#complete-task").after('<div id = "transp"></div>'),
	$(".task_action_tabs").hide(),
	$("[value=Release Ownership]").hide().after('<span class="re">Confirm Runbook Execution</span>'),
	$("#transp").html("<div class='b-popup-content'><p class='popupQuestion'>Has the runbook been followed?</p><button class='runbookYes'>&#10004; | YES</button><button class='runbookNo'>&#x2718; | NO</button><br/><textarea class='popupTextarea'></textarea><br/><button class='runbookGo'>Log Runbook Execution</button></div>")),
	$(".runbookYes").click(function(){$(".popupTextarea").val("Runbook Executed:\n\n").show("slow"),$(".runbookGo").show("slow")}),$(".runbookNo").click(function(){$(".popupTextarea").val("Runbook NOT Executed:\n\n").show("slow"),$(".runbookGo").show("slow")}),$(".runbookGo").click(function(){$(".task_action_tabs").show("slow"),$("#transp").hide("slow"),a=$(".popupTextarea").val(),$('textarea[name="dbfield:3:__raw_custom__:__single__:log_work_work_log"]').val(a),$('input[name="dbfield:3:__raw_custom__:__single__:log_work_time_spent"]').val("3m"),$("[value=Log Work]").click()})}
if( // view-tasks
...
console.log('ISC_Core.js has been loaded...');